    ...
    def run(self, state: TradingState) -> Dict[str, List[Order]]:
        #try:
            ...
            for product in self.PROD_LIST:
                ...
                if product == "BANANAS":
                    if self.start_trade_tf[product] and theo_p != 0 and self.price != 0 and len(self.hist_prices[product]) > self.BANANA_AVG_INTERVAL:
                        bid_price,ask_price = self.get_optimal_bid_ask_price(product, theo_p)
                        spread = ask_price - bid_price
                        certainty = spread / self.max_trade_spread[product] if self.max_trade_spread[product] > 0 else 0
                    ...
                    if self.start_trade_tf[product] and theo_p != 0 and self.price != 0:
                        bid_price,ask_price = self.get_optimal_bid_ask_price(product, theo_p)
                        spread = ask_price - bid_price
                        certainty = spread / self.max_trade_spread[product] if self.max_trade_spread[product] > 0 else 0
                        remain_bid_size = self.get_max_bid_size(product)
